...
def not_availbel_yet(request):
	return render(request, "not_availbel_yet.html", {})	
...
def tryit(request):
	return render(request, "auth/fuck5f.html", {})


def pick_model(request):
	ai_models = AiModel.objects.all()

	return render(request, "chat/all-models.html", {'ai_models':ai_models})	


# def chat_model(request):
# 	return render(request, "chat/chat-model.html", {})	


def model_page(request, ai_model_name):
	ai_model = AiModel.objects.get(name=ai_model_name)

	return render(request, "chat/model-page.html", {'ai_model':ai_model})



def start(request, ai_model_name):
	other_user = User.objects.get(username = 'ai_user')
	my_chatrooms = request.user.chat_groups.filter(is_private=True)
	chosen_ai_model = AiModel.objects.get(name=ai_model_name)
	...
	if my_chatrooms.exists():
		for chatroom in my_chatrooms:
			if other_user in chatroom.members.all():
				...
				chatroom = ChatGroup.objects.create(is_private = True, ai_model=chosen_ai_model)
	...
	if chosen_ai_model:
		chatroom.ai_model= chosen_ai_model
		chatroom.save()

		messages.success(request, f'you have selected: {chatroom.ai_model.name}', extra_tags='safe')
		return redirect('chatroom', chatroom.group_name)

	else:
		messages.error(request, 'some thing went wrong! please try again.', extra_tags='safe')	
		return redirect('model-page', ai_model_name)




@login_required 
def chat_model(request, chatroom_name='public-chat'):
    ...
    chat_ai_model= chat_group.ai_model   
    ...
    return render(request, "chat/chat-model.html", context)
...
def login_usr(request):
	if request.method== 'POST':
		...
		if user is not None:
			...
			return redirect('pick-model')

		else:
			messages.error(request, ('there was an error, please try again'))
			return redirect('login')	

	else: return render(request, "auth/login_usr.html", {})	
